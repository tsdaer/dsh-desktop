...
`apps/desktop/scripts/tauri-ui-smoke.mjs` 提供显式的 Linux x64 deb 构件检查。它用 `dpkg` 安装 package,启动 `tauri-driver`,为已安装可执行文件创建 W3C WebDriver session,并驱动原生 WebKit WebView,直到 `data-composer-seat` 与 `data-composer-input` 元素表明当前 Lexical composer 已就绪。在启动页切换到主窗口期间,没有就绪主窗口时会继续枚举窗口;已选择的启动页或 WebView 在下一次请求前关闭时,只重试 WebDriver 的 `no such window` 结果。检查会把 `snapshots/web/navigation-panes/` 下已记录的会话 fixture 中代数最高的一份——`session.jsonl` 或其 `session.vN.jsonl` 后继——还原到临时 `DSH_HOME`,其代数选择方式与 Web lane 自身的 scaffold 一致;Web fixture 是录制结果而不是可以直接持久化的构件,因此 smoke 会用该代的格式版本、稳定 id、seed 标记、cwd、delegation depth 和 agent preset 重建 header,并以与该校 header 配对的按代命名日志(`session.jsonl` 表示 v0,其后为 `session.vN.jsonl`)落盘——名称与 header 不一致的日志会被拒绝。检查会确认全新 home 的内测声明;若 API-key onboarding 出现就推迟该步骤,若稳定缺席则表示当前组合没有需要关闭的步骤。检查只在本地化的 Sessions（会话）树中导航,防止另一个已挂载的树提供错误 row。它会按需展开 fixture 所在的 Workspace 或 Ungrouped row,忽略临时 New Session row,再打开唯一的非空持久化 session,无论启动时该 session 是否已经被选中,展开模型面对的 Bash 终端卡片,要求出现 `NAVIGATION_OK`,并可保存 WebDriver 截图。导航失败会报告 session tree、持久化候选、分组与 row 的数量和标签,保留当前 WebView 截图,并打印有界的 driver 输出和隔离的原生 splash 日志。`DSH_PATCH` 会让桌面壳把临时 plaintext JSONL persistence 覆盖传给 Web profile;生产 bundle 保持不变。已安装进程会收到 display 和普通启动环境,但会删除名称中包含 `KEY`、`SECRET`、`TOKEN` 或 `PASSWORD` 的环境变量。组装后的 Web 回放继续保留内容索引搜索检查,因为它的 fixture 通过负责索引对账的后端 API 植入。
