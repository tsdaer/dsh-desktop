...
`apps/desktop/scripts/packaged-smoke.spec.mjs` covers the installed deb sidecar and runtime paths and rejects duplicate installed sidecars. The full deb installation, launch, terminal, purge, and user-data checks remain target-native workflow evidence.

## Consequences

Linux installation evidence observes the files that the package manager installed rather than a second extracted copy. A broken package file list, missing or duplicate installed sidecar, or missing runtime now fails with a specific error before the launch check. This change does not establish minimum-distribution, updater, or GUI evidence.
...
**Derive sidecar and runtime paths from fixed `/usr/bin` and `/usr/lib` prefixes.** Rejected because the package file listing is the package manager's authoritative installed-path record and gives a direct failure when the layout changes.
