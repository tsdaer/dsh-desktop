...
内置裸插件名通过修复后的 `$DSH_HOME/profiles/node_modules` 回退目录解析,其中的链接指向打包运行时。打包桌面启动默认不设置 `DSH_BARE_MODULE_BASE`,使 profile 安装的 bundle 从 profile 自己的 `node_modules` 解析;由宿主拥有完整插件集的场景仍可显式设置。 [profile 自有 bundle 解析修正](../bug-fix/2026-08-20-desktop-profile-bundle-resolution.zh.md)记录了打包默认值不能使用仅运行时锚点的原因。
