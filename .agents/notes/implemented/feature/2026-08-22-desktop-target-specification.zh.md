...
[English](2026-08-22-desktop-target-specification.md) | 中文
...
每一行目标规格负责 Node 发行版名称与压缩包类型、精确的 sidecar 源成员、带目标后缀的暂存文件名、Tauri 安装后的 sidecar 文件名、原生平台键、bundle 类型、产物目录、更新器后缀、按目标区分的运行时目录和大小预算。sidecar 获取与运行时烘焙消费暂存文件名;安装版启动与安装包检查消费安装后的文件名。大小报告和 bundle 编排消费同一个不可变规格。压缩包成员在拼接到临时解压目录之前会先经过路径校验。
...
`apps/desktop/scripts/target-spec.spec.mjs` 固定了三行目标规格的全部识别字段,其中包括两种 sidecar 文件名,拒绝缺失或不支持的目标,验证三种 Node 压缩包布局,并拒绝压缩包路径穿越。Node 脚本语法检查通过,`git diff --check` 通过。
