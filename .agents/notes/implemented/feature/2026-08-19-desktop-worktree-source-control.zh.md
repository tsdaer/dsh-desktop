...
Source Control 要求桌面运行时可以使用 Git；发现或读取状态失败时报告不可用。仓库状态是快照，后续刷新可能观察到不同的更改；写入操作在变更前会对照新读入的状态清单重新校验（见[桌面 Source Control 写入](2026-08-21-desktop-source-control-actions.md)）。NUL 分隔的 porcelain 解析保留包含空格或换行的路径；不支持或不安全的记录只通过有界的状态分类可见，绝不会成为文件系统权限来源。
