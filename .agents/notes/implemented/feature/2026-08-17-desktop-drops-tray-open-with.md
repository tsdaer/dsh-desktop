...
**Tray and close behavior.** The `tray-icon` feature joins the tauri dependency; `setup_tray` builds a tray with the bundled window icon and a two-item menu (显示主窗口 / 退出). Left-click on the icon shows and focuses the main window; 退出 stops the runtime child explicitly and calls `app.exit(0)`. The 桌面设置 section offers an explicit choice between direct exit and retaining the application in the tray, persisted as `closeToTray` in the bridge settings namespace (`$DSH_HOME/settings.yaml` via `POST /dsh-bridge/policy`). The bridge client mirrors the durable value into Rust through the `set_close_to_tray` command on boot and on every change; the main window's `CloseRequested` handler prevents the close and hides the window for the tray choice. Direct exit is the default; the tray is always present regardless.

**Explorer open-with.** On every start the shell (re)registers per-user context-menu entries under HKCU via `reg.exe` (no elevation, idempotent, always pointing at the current exe): `Software\\Classes\\Directory\\shell\\dsh-desktop` (folder row) and `Software\\Classes\\Directory\\Background\\shell\\dsh-desktop` (folder background), label 以 dsh-desktop 打开, command `"<exe>" "%V"`. `tauri-plugin-single-instance` makes the first process authoritative; a later invocation queues its canonical directory in that process, focuses the existing window, emits a wake event, and exits. The bridge client drains the queue after its listener is installed, then selects the Workspace whose canonical path is the longest ancestor of the directory. It opens the Workspace's most recent session or starts one when none exists. For an unmatched directory, the page asks the user before calling `workspaces.create({ path })` and opening the result. Registration failures are logged, never fatal; the uninstaller removes both keys through the `NSIS_HOOK_POSTUNINSTALL` macro in `apps/desktop/src-tauri/installer-hooks.nsh`.
...
**Registering the context menu from NSIS** — rejected: first-run registration keeps the command current across dev runs and reinstalls. Only removal belongs to the installer, through the uninstall hook, because the application cannot delete its own keys as it is being uninstalled.
...
Drops now give the model host paths outside the workspace sandbox — the filesystem policy governs what the agent may read, and the old copy-into-`drops/` guarantee is gone. Images keep full composer intake. The drop policy rows (copy switch, size cap) are removed from the settings page; 调试模式 remains. The tray is always present; the close choice decides whether the title-bar button exits or hides. Explorer launches converge on one application process and may create a Workspace after confirmation. Uninstalling removes the per-user registry keys; an update reinstall removes them too and the next launch re-registers them. Dev boots refresh the profile bridge with a directory copy (no npm); the dev flow additionally requires the repository's workspace libs built (`pnpm run build:lib`), because the dev profile's module fallback points at the checkout.

## Verification

`cargo build` and the bridge tsc + tsdown builds pass. An end-to-end boot of the real web profile in a throwaway DSH_HOME (bridge packages copied in, patch rows merged) answered `GET /dsh-bridge/config` with the new shape, `POST /dsh-bridge/policy {closeToTray:true}` persisted `desktop-bridge.closeToTray` into settings.yaml and the follow-up `/config` read it back, and `/dsh-bridge/balance` stayed normalized. After the workspace libs were built, the repo CLI boots the real web profile to its readiness line (`dsh web: http://127.0.0.1:<port>`). A real second invocation with a nested directory exited with code 0, left one desktop process, focused the existing window, and routed to the owning Workspace without prompting; an unmatched canonical directory reached the confirmation text without the Windows verbatim-path prefix. The tray, close interception, and context-menu registration exercise on app start; the registry entries were written during the first real run after the HKCU prefix fix.
