...
`apps/desktop/scripts/packaged-smoke.spec.mjs` covers macOS argument validation, app-bundle executable resolution, dmg mount and native installation arguments, canonical installation-root resolution, isolated splash diagnostics, and the existing Linux paths. The target-native macOS jobs invoke the same script against the generated app bundle.
