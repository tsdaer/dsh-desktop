...
In the Tauri shell, Explorer and Search send normalized Workspace-relative file requests to a native preview-window command. The shell validates the Workspace id and path again, derives a hashed `preview-*` label from both values, and hands creation or refocus to Tauri's asynchronous runtime before that task enters the main UI loop. The invoking WebView IPC therefore returns before dynamic WebView creation starts, so native window setup cannot block its own navigation. Each new WebView loads the authenticated root with the process launch token plus `dsh_preview=1`, `workspaceId`, and `path`; the Host exchanges that token for the window's signed cookie and redirects to the same preview query without the launch token. The separate per-boot bridge bearer token is returned through a scoped command and never enters the preview URL. New windows become visible and receive focus as soon as native creation succeeds, then receive focus again after their first page load. The preview read is cancelable and has a bounded timeout that becomes an in-window error. Its lazy Web entry loads the shared shell defaults, design tokens, typography, scrollbar, and Shiki palette so source-code token colors resolve without activating the full client plugin roster. A responsive document surface presents the file name, relative path, type, and close action above a centered Markdown canvas or full-width code card; loading, refusal, and truncation have separate visual states. The preview follows the main window's resolved theme: the bridge client forwards every `theme/change` snapshot through a shell command that broadcasts it to each live preview WebView and replays the current snapshot when a preview page finishes loading; a plain browser session falls back to the operating system color preference. Browser runs without Tauri retain the existing in-pane viewer as a fallback.
...
Preview-window tests pin early path validation, normalized request arguments, locale forwarding, unavailable-Tauri fallback, native command failure fallback, Rust path rejection, and Workspace-plus-path label scoping. The desktop shell build checks the Tauri window and command registration. The Web build checks the minimal preview entry alongside the regular Web entry and emits its responsive document styles, shared theme variables, and Shiki palette in the lazy stylesheet. GUI evidence checks a real built preview entry at desktop dimensions with highlighted source content.
