...
The Host sorts directories before files, applies configurable entry, UTF-8 JSON byte, and elapsed-time limits, and reports projection truncation explicitly. HTTP request aborts cancel the filesystem operation. The bridge configuration owns positive safe-integer limits with defaults of 256 entries, 131072 response bytes, and 5000 milliseconds.
