...
Remaining work is the target-native Linux terminal UI/model-facing workflow, installed-update, minimum-baseline, and packaged GUI evidence; macOS credentialed execution of the opt-in signing lane, notarization/Gatekeeper evidence, installed-update smoke, supported-release documentation, and GUI evidence; plus final Windows packaged-installer regression evidence. Signed-lane automation, manifest signature verification, and packaged PTY smoke do not replace the required installed-version update smokes or user-visible GUI evidence.

The current Windows checkout passes the desktop script tests (43 tests), desktop frontend tests (58 tests), Tauri Rust tests (5 tests), the desktop release workflow structure test, the named bilingual-pair checks, and Agent Note format validation. `pnpm run doc-sync` passes 27 gates; its documentation build remains blocked by the checkout's missing `vue/server-renderer` dependency rather than by the desktop documents.
