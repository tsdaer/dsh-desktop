...
The [Linux installed update acceptance workflow](../../implemented/testing/2026-08-23-desktop-installed-update-workflow.md) now accepts two immutable tags, builds version N with a fixed loopback endpoint, builds and signs version N+1 on the same `ubuntu-24.04` runner, and invokes the target-native update smoke with the packaged PTY probe. It uploads the smoke log without mutating a Release; actual workflow execution remains open evidence.
