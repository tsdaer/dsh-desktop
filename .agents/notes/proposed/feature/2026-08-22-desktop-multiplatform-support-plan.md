...
Remaining work is target-native Linux terminal, installed-update, minimum-baseline, and packaged GUI evidence; macOS native packaged smoke, signing/notarization, updater artifact publication, installed-update smoke, documentation, and GUI evidence; plus final Windows regression evidence. Manifest signature verification is implemented, but it does not replace the required installed-version update smokes.
