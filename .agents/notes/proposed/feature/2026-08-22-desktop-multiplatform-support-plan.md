...
The updater manifest generator now accepts `--download-base-url` for controlled update fixtures. It validates the HTTP(S) base, rejects query strings, fragments, and path-bearing release names, and keeps the GitHub Release URL as the default; [the controlled manifest URL note](../../implemented/testing/2026-08-22-desktop-update-smoke-manifest-base.md) records the mechanism. This prepares a local signed fixture but does not provide endpoint injection, installed N-to-N+1 replacement, relaunch, or user-data evidence.
