...
The updater manifest generator now accepts `--download-base-url` for controlled update fixtures, and `bundle` accepts `--updater-endpoint` to embed a runner-local endpoint in a temporary Tauri configuration layer. Both options validate the endpoint without changing the production GitHub URL; [the controlled manifest URL note](../../implemented/testing/2026-08-22-desktop-update-smoke-manifest-base.md) and [the endpoint injection note](../../implemented/testing/2026-08-22-desktop-update-smoke-endpoint-injection.md) record the mechanisms. Installed N-to-N+1 replacement, relaunch, user confirmation, and user-data evidence remain open.

Remaining work is the target-native Linux terminal UI/model-facing workflow, installed-update, minimum-baseline, and packaged GUI evidence; macOS credentialed execution of the opt-in signing lane, notarization/Gatekeeper evidence, installed-update smoke, supported-release documentation, and GUI evidence; and native Windows runner execution of the installed-package regression. Signed-lane automation, manifest signature verification, and packaged PTY smoke do not replace the required installed-version update smokes or user-visible GUI evidence.

The current Windows checkout passes the desktop script tests (47 tests), desktop frontend tests (58 tests), Tauri Rust tests (5 tests), the desktop release workflow structure test, the named bilingual-pair checks, and Agent Note format validation. `pnpm run doc-sync` passes all 28 gates, including the documentation build.
