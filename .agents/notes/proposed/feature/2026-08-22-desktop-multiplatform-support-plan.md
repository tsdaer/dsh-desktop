...
Remaining work is the target-native Linux terminal UI/model-facing workflow, installed-update, minimum-baseline, and packaged GUI evidence; macOS credentialed execution of the opt-in signing lane, notarization/Gatekeeper evidence, installed-update smoke, documentation, and GUI evidence; plus final Windows regression evidence. Signed-lane automation, manifest signature verification, and packaged PTY smoke do not replace the required installed-version update smokes or user-visible GUI evidence.
