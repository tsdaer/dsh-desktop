...
The current Windows checkout passes 59 desktop script tests, 7 dedicated desktop release/update workflow structure tests, the named bilingual-pair checks, Agent Note format validation, and `pnpm run lint`. `pnpm run doc-sync` passes all 28 gates, including the documentation build. A retained local NSIS artifact reaches the installer stage without the inherited-stdio failure but does not reach readiness, so native Windows packaged evidence remains open.
