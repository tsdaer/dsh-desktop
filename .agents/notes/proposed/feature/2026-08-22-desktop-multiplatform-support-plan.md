...
The packaged smoke preserves process command lines, identifies a re-parented installed Node sidecar by its resolved path, and bounds graceful shutdown before a forced cleanup attempt. [Desktop packaged process cleanup verification](../../implemented/bug-fix/2026-08-22-desktop-packaged-process-cleanup.md) records this check; it strengthens process-cleanup evidence without closing the remaining native-platform acceptance work.

The packaged smoke now also accepts `--terminal-smoke`. After launching an NSIS, AppImage, deb, app, or dmg artifact, it resolves exactly one Tauri-installed sidecar and runtime from the package and uses that runtime to execute a fixed shell-native marker probe through `node-pty`, then waits for probe exit and performs target-local cleanup. The Windows, Linux, and macOS workflow jobs invoke this check. [Desktop packaged terminal smoke](../../implemented/testing/2026-08-22-desktop-packaged-terminal-smoke.md) records the evidence and its limit: this proves packaged PTY bytes and disposal, not the browser/model-facing terminal workflow.

The Linux release job also runs `packaged-smoke --web-smoke` against the installed deb package. Chromium opens the installed package's readiness URL, waits for the composer and textarea, validates the document title, and uploads an optional screenshot. [Desktop installed-package web UI smoke](../../implemented/testing/2026-08-23-desktop-installed-package-web-smoke.md) records the evidence boundary: this proves the installed shell/runtime serves and renders the Web UI, not native Tauri WebView behavior or the model-facing terminal workflow.

The Linux release job now also installs Chromium and replays the keyless [`navigation-panes` terminal UI scenario](../../implemented/testing/2026-08-23-desktop-linux-terminal-ui-replay.md) through the assembled Web profile under `xvfb-run`. The scenario reconstructs the recorded session with the standard preset, verifies the model-facing terminal card alongside navigation and clipboard behavior, and uploads its log and failure screenshot. Windows skips the Bash-specific assertions because the standard preset selects PowerShell there. This closes the assembled-Web terminal UI evidence gap only; it does not establish installed Tauri WebView behavior, packaged GUI evidence, or Linux support.

The Linux release job also runs [`tauri-ui-smoke`](../../implemented/testing/2026-08-23-desktop-linux-native-tauri-ui-smoke.md) against the installed deb package under `xvfb-run`. The smoke drives the native WebKit WebView through `tauri-driver`, restores the committed keyless session fixture, verifies the composer and model-facing terminal card, captures a screenshot, purges the package, and checks user-data retention. It closes the installed Tauri WebView terminal-card evidence gap; minimum-distribution, installed update, and the complete packaged GUI checklist remain open.

The native UI smoke now bounds `tauri-driver` cleanup: it recognizes an exit recorded before listener registration, escalates from `SIGTERM` to `SIGKILL` after a grace period, and continues package purge and user-data checks before reporting a surviving driver. [The native UI smoke process cleanup note](../../implemented/testing/2026-08-23-desktop-native-ui-process-cleanup.md) records this harness invariant; target-runner execution remains required.

The deb installation smoke queries the package-manager file list after installation, launches the executable registered by `dpkg`, and runs its optional terminal probe against the installed sidecar and runtime. [Desktop deb installed-runtime smoke](../../implemented/bug-fix/2026-08-23-desktop-deb-installed-runtime-smoke.md) records this path-selection fix; native Linux installation, installed-package terminal UI, update, uninstall, baseline, and GUI evidence remain open.
...
The Windows x64 release job now runs the [Windows installed-package smoke](../../implemented/testing/2026-08-22-desktop-windows-packaged-smoke.md) after size and artifact checks and before upload. It installs the NSIS artifact into a disposable directory, launches the installed shell, optionally runs the packaged PTY probe, verifies managed-process cleanup, runs the uninstaller, and checks temporary user data retention. The smoke tolerates inherited stdio from installer and uninstaller commands, and its regression test covers the `spawnSync` path where captured stdout is `null`; the [inherited-stdio note](../../implemented/bug-fix/2026-08-23-desktop-packaged-smoke-inherited-stdio.md) records the failure mode. The workflow and script tests cover the mechanism; native Windows runner execution remains the evidence source.
...
The [Linux installed update acceptance workflow](../../implemented/testing/2026-08-23-desktop-installed-update-workflow.md) now accepts two tag refs, resolves and records their commit snapshots before reading version data, verifies the initial checkout and each build checkout against those snapshots, builds version N with a fixed loopback endpoint, builds and signs version N+1 on the same `ubuntu-24.04` runner, and invokes the target-native update smoke with the packaged PTY probe. It uploads the smoke log without mutating a Release; actual workflow execution remains open evidence.

The [macOS installed update acceptance workflow](../../implemented/testing/2026-08-23-desktop-macos-installed-update-workflow.md) now accepts two tag refs and a fixed loopback port, verifies their commit snapshots, builds and signs both arm64 versions on `macos-14`, notarizes and staples each app and dmg, stages the signed next-version inventory, and invokes the target-native update smoke with the packaged PTY probe. It uploads only the smoke log and removes its temporary signing keychain; execution with product-owned Apple and updater credentials remains open evidence.

The [Windows installed update acceptance workflow](../../implemented/testing/2026-08-23-desktop-windows-installed-update-workflow.md) now accepts two tag refs and a fixed loopback port, verifies their commit snapshots, builds and signs both x64 versions on `windows-latest`, stages the signed next-version inventory, and invokes the target-native update smoke with the NSIS installation mode and packaged PTY probe. It uploads only the smoke log and does not mutate a Release; native execution remains open evidence.

The desktop release workflow checks out `needs.validate.outputs.commit` in every source-reading job and verifies `git rev-parse HEAD` before building, staging, or attaching artifacts. The structural test covers all six jobs; this closes source-snapshot consistency but does not add target-native runtime evidence. The [validated-commit Agent Note](../../implemented/process/2026-08-23-desktop-release-uses-validated-commit.md) records the release invariant.

Remaining work is execution of the installed Tauri Linux terminal UI/model-facing workflow, execution of the installed-update acceptance workflow, minimum-baseline, and packaged GUI evidence; macOS credentialed execution of the opt-in signing lane, notarization/Gatekeeper evidence, installed-update runner evidence, supported-release documentation, and GUI evidence; and native Windows runner execution of the installed-package regression. The update smoke driver, signed-lane automation, manifest signature verification, packaged PTY smoke, and assembled-Web replay do not replace target-native installed-version update execution or user-visible GUI evidence.

The focused validation for this work package passes 65 desktop script tests, 6 Rust unit tests, and 7 dedicated desktop release/update workflow structure tests. The native UI cleanup tests cover early driver exit, graceful termination, and bounded escalation. The Windows Web replay passes five tests and skips three Bash-only Linux assertions; the named bilingual-pair checks, Agent Note format validation, `pnpm run lint`, and `pnpm run doc-sync` also pass, with doc-sync completing all 28 gates including the documentation build. These checks do not replace target-native installed-package evidence; a retained local NSIS artifact reaches the installer stage without the inherited-stdio failure but does not reach readiness.
