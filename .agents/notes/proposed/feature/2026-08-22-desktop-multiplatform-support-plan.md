...
Remaining work is target-native Linux terminal, installed-update, minimum-baseline, and packaged GUI evidence; macOS native packaged smoke, credentialed execution of the opt-in signing lane, notarization/Gatekeeper evidence, installed-update smoke, documentation, and GUI evidence; plus final Windows regression evidence. Signed-lane automation and manifest signature verification do not replace the required installed-version update smokes.
