...
The Linux-only `pnpm --filter @deepseek-ai/dsh-desktop native-ui-smoke -- --target x86_64-unknown-linux-gnu --artifact <deb> --screenshot <path>` installs the deb, starts `tauri-driver` against the installed executable, restores the committed keyless `navigation-panes` session, opens it through the native WebKit WebView, verifies the composer and model-facing terminal card, and captures a screenshot before purging the package. Its search first waits for a guaranteed miss to reconcile the cold SQLite content index, then finds the seeded transcript marker. The runner needs `webkit2gtk-driver`, `tauri-driver`, and an X display such as `xvfb`; the fixture uses a temporary plaintext session-persistence override and must not be treated as live model or minimum-distribution evidence.
