...
The Linux-only `pnpm --filter @deepseek-ai/dsh-desktop native-ui-smoke -- --target x86_64-unknown-linux-gnu --artifact <deb> --screenshot <path>` installs the deb, starts `tauri-driver` against the installed executable, restores the committed keyless `navigation-panes` session, expands its sole collapsed group, opens the sole persisted row from the main session tree through the native WebKit WebView, verifies the composer and model-facing terminal card, and captures a screenshot before purging the package. The assembled Web replay owns content-index search coverage. The runner needs `webkit2gtk-driver`, `tauri-driver`, and an X display such as `xvfb`; `DSH_PATCH` passes the temporary plaintext session-persistence overlay through the desktop shell to the Web profile, and a failed WebDriver assertion retains the current WebView screenshot when a session exists. The fixture must not be treated as live model or minimum-distribution evidence.
