...
Target-native package startup smoke is available through `pnpm --filter @deepseek-ai/dsh-desktop packaged-smoke -- --target <triple> --artifact <path>`. Linux accepts an AppImage or, with `--install-deb`, a deb; macOS accepts an app bundle or, with `--install-dmg`, a dmg. The smoke launches the packaged executable, waits for the runtime readiness URL, verifies that managed descendants exit, and checks that the temporary `DSH_HOME` remains after package removal. It requires the target runner and does not replace terminal, updater, minimum-distribution, or GUI evidence.
