...
Env wiring in main.rs: DSH_CLI/DSH_NODE/DSH_BARE_MODULE_BASE/DSH_BRIDGE_TARBALL win (dev launcher); a release build without DSH_CLI requires resources/runtime/lib/bin.js and the target-named sidecar beside the app executable, then performs offline bridge copying. A packaged launch leaves DSH_BARE_MODULE_BASE unset by default so the profile can resolve user bundles; an explicit value remains available for hosts that own the complete plugin set.
...
An update-smoke fixture may pass `--download-base-url <http(s)://host/path>` to `scripts/updater-manifest.mjs` so a target runner can serve signed artifacts locally; the option rejects query strings, fragments, and path-bearing release names, and does not change the production GitHub endpoint. The installed version-N to version-N+1 workflow still needs target-specific endpoint injection and native installation evidence.
