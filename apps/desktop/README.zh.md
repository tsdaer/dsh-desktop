...
仅限 Linux 的 `pnpm --filter @deepseek-ai/dsh-desktop native-ui-smoke -- --target x86_64-unknown-linux-gnu --artifact <deb> --screenshot <path>` 会安装 deb,针对已安装可执行文件启动 `tauri-driver`,还原已提交的无 key `navigation-panes` session,展开它唯一的折叠分组,再通过原生 WebKit WebView 从主会话树打开唯一的持久化 row,校验 composer 与模型面对的终端卡片,并在 purge 安装包前截图。内容索引搜索覆盖由组装后的 Web 回放负责。runner 需要 `webkit2gtk-driver`、`tauri-driver` 和 `xvfb` 等 X display;`DSH_PATCH` 会让临时 plaintext session-persistence 覆盖经桌面壳传给 Web profile,而 WebDriver 断言失败时会在 session 已建立的情况下保留当前 WebView 截图。fixture 不能当作真实模型或最低发行版证据。
