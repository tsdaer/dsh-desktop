...
- `GET /workspace` —— 把壳子启动时携带的文件夹对照工作区注册表解析(见“以 dsh-desktop 打开”)。

桥接 client 半边在页面上负责壳侧行为:上面的拖放处理、关闭到托盘的镜像、调试守卫,以及启动文件夹的工作区跳转。

## 桌面设置、托盘与关闭行为

dsh 设置页的 桌面设置 分区(由桥接 client 注册)有两行,都经桥接 host 路由持久化:

- 关闭行为:标题栏关闭按钮是真实退出,还是最小化到系统托盘。开启后,关闭窗口(按钮或 Alt+F4)会阻止关闭并隐藏窗口;运行时继续服务,会话继续运行。托盘菜单的 退出 才是真正的退出(它会停掉运行时子进程并结束应用)。
...
菜单运行 `<exe> <文件夹>`。启动时壳子把文件夹以 `DSH_DESKTOP_OPEN` 传给运行时,桥接 host 的 `GET /dsh-bridge/workspace` 对照工作区注册表解析:精确匹配工作区路径,或按规范路径祖先检查匹配工作区内的任意子目录。桥接 client 随后在页面工作区基线就绪后跳到该工作区 —— 打开其最近的会话,没有会话则新建一个。不在任何工作区内的文件夹(或未带文件夹的普通启动)按正常流程启动。

注册是尽力而为的,失败只记日志;NSIS 安装器目前卸载时不移除这些注册表键。

## 测试版范围

- dev 用 PATH 上的 'node' 跑仓库构建出的 CLI;打包应用自带 Node sidecar 和烤出的运行时(见上文 打包 / 打包运行时)。剩余缺口:无自动更新、无单实例锁,且 Windows-only sidecar 意味着 Linux/macOS 未处理(dsh 依赖树里 node-pty 也没有 Linux 预编译)。
