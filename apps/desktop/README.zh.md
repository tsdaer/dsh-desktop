...
deb 冒烟会在安装前创建用户数据标记,并要求执行 package purge 后该标记仍然存在。这样可以验证安装包的卸载路径,同时不把一次性 smoke home 当成应用应拥有的数据。它会在足以容纳烘焙运行时的有界输出配额内读取 package 自有文件清单,并复用该清单查找可执行文件、sidecar 和运行时。生产运行时烘焙会移除工作区的开发依赖,因此 Linux release job 会在安装 Chromium 以及运行安装包、原生 UI 和回放探针前恢复锁定的开发依赖安装。该 job 还会在 Chromium 下回放无 key 的 `navigation-panes` 浏览器场景,覆盖组装 Web profile 的模型面对的终端卡片和导航交互。这项回放与已安装包 smoke 分开,不代表已安装 GUI 证据;版本 N 到 N+1 的已安装更新仍需要目标原生验收 workflow。
