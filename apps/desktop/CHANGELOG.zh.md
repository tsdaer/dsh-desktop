...
- 工作区"以 dsh-desktop 打开"的路由重新对齐当前客户端 store 契约：工作区列表的就绪生命周期（`phase`）与经由 `uiWorkspace` 导航服务的会话启动。转发的目录重新可以切换到所属工作区并打开其最近的会话，当没有工作区拥有该目录时会提示注册工作区。
- 当会话激活失败的错误文本符合"已安装插件访问了本构建不再提供的上下文成员"这一特征时，恢复与创建的失败消息及其错误详情现在都会附带诊断提示，指明第三方插件不兼容是最可能的原因。
